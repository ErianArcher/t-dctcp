...
class Fattree(Topo):
	...
	def createLinks(self, bw_c2a=10, bw_a2e=10, bw_e2h=10, **lconfig):
		"""
			Add network links.
		"""
		lc2a_config = lconfig
			...
			for i in xrange(0, end):
				for j in xrange(0, end):
					self.addLink(
						...
						**lc2a_config)   # use_htb=False

		# Agg to Edge
		la2e_config['bw'] = bw_a2e
		for x in xrange(0, self.iAggLayerSwitch, end):
			for i in xrange(0, end):
				for j in xrange(0, end):
					self.addLink(
						self.AggSwitchList[x+i], self.EdgeSwitchList[x+j],
						**la2e_config)   # use_htb=False

		# Edge to Host
		le2h_config['bw'] = bw_e2h
		for x in xrange(0, self.iEdgeLayerSwitch):
			for i in xrange(0, self.density):
				self.addLink(
					self.EdgeSwitchList[x],
					self.HostList[self.density * x + i],
					**le2h_config)   # use_htb=False
